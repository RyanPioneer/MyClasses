...
public class TGraph {
	...
	public void insertReplication(TPartStoredProcedureTask task, HashSet<PrimaryKey> replicatedKeys, int assignedPartId) {
		// QMurphy: 這邊為什麼 reRoute 是 false
		TxNode node = new TxNode(task, assignedPartId, false);
		txNodes.add(node);
		
		for (PrimaryKey res : replicatedKeys) {
			Node target = getResourcePosition(res);
			node.addReadEdges(new Edge(target, res));
			target.addWriteEdges(new Edge(node, res));
		}
	}

	// QMurphy: reRoute 這邊的用途是什麼？之後會用來幹嘛？
